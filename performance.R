...
results_combined <- rbind(results, do.call(rbind, results_2_3))
...
ggplot(results_combined, aes(x=rank,y=CI_width,group=rank,fill=rank)) +
  geom_boxplot()
...
range_of_upper_bounds <- results_combined |>
  # filter(dataset == 1) |>
  group_by(dataset) |>
...
range_of_upper_bounds/mean_width
